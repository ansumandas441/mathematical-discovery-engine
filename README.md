...
An automated theorem discovery engine built on a knowledge graph of mathematics. It encodes 6,225 nodes (axioms, states, theorems, techniques) and 10,556 edges into a searchable directed graph, then uses an orchestrator/worker LLM architecture to find proofs by composing 62 named techniques.

The companion report traces ~100 of the most consequential theorems — from Pythagoras through Perelman — with the discovery context, motivation, and thought process behind each.
